    ...
    <div class="flex justify-center gap-7">
        @if ($totalPrice !== 0)
                            ...
                            <div class="flex-col justify-between">
                                <div class="flex justify-between border-[1px] ">
                                    <form action="{{ route('cart.update', ['id' => $product->id]) }}" method="POST" class="flex">
                                        ...
                                        <button class="flex text-center items-center justify-center w-8 h-w-8 rounded-lg transition-all duration-300 active:scale-95 text-white hover:brightness-125" type="submit">
                                            <svg class="w-4 h-4 text-black" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    ...
                                    <form action="{{ route('cart.update', ['id' => $product->id]) }}" method="POST" class="flex">
                                        ...
                                        <button class="flex text-center items-center justify-center w-8 h-w-8 rounded-lg transition-all duration-300 active:scale-95 text-white hover:brightness-125" type="submit" name="action" value="increment">
                                            <svg class="w-4 h-4 text-black" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                ...
                                <form action="{{ route('cart.delete', ['id' => $product->id]) }}" method="POST" class="">
                                    @csrf
                                    <button class="hover:text-red-500 flex border-x-[1px] border-b-[1px] text-center items-center justify-center w-full" type="submit" name="action" value="delete">
            ...
            <div class="rounded-md flex-col h-fit flex my-5 px-5 py-4 bg-white text-sm border-[1px]">
                ...
                <button class="bg-yellow-300 mt-12 w-full h-9 font-bold hover:brightness-95 uppercase" type="button" id="checkoutButton">
                    CHECKOUT
                </button>
            </div>
        @else
            <div class="container flex mx-auto my-8 p-5 pl-8 bg-white rounded-md shadow-md w-11/12 flex-col justify-between">
