        ...
        <section class="bg-slate-300 mb-8">
            ...
            <div class="bg-slate-400">
               <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                <a href="/product/{{ $product->id }}" class="transform hover:scale-105 transition-transform duration-300 ease-in-out">
                    <div class="bg-white p-4 rounded-md shadow-md max-h-96">
                        <img src="{{ asset('storage/'.$product->gambar) }}" alt="Product 1" class="w-full h-40 object-cover mb-4">
                        ...
                        <span class="text-blue-500 font-semibold">Rp. {{ $product->harga }}</span>
                        <form action="{{ route('cart.add', $product->id) }}" method='POST'>
                            @csrf
                            <input type="hidden" value="{{ $product->id }}" name="id_product">
                            <input type="hidden" value="1" name="jumlah">
                            <button type="submit" class="bg-red-600">Add to Cart</button>
                        </form>
                    </div>
                </a>
                @endforeach
